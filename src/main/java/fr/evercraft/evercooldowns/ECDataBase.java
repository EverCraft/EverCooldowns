...
public class ECDataBase extends EDataBase<EverCooldowns> {
	...
	public void remove() {
		...
		try {
			...
			this.plugin.getELogger().info("Deletions of expired cooldowns");
		} catch (SQLException e) {
	    	this.plugin.getELogger().warn("Error deletion of expired cooldowns : " + e.getMessage());
	...
	public void clearAll() {
		...
		try {
			...
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.execute();
			this.plugin.getELogger().info("Deletions from the database");
		} catch (SQLException e) {
	    	this.plugin.getELogger().warn("Error deletions from the database : " + e.getMessage());
		} catch (ServerDisableException e) {
			e.execute();
		} finally {
			try {
